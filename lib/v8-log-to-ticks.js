...
function fixLines (line) {
  ...
  if (nodeMajorV === 10) {
    ...
    // Replace any \\ that's not part of a unicode or ASCII escape character
    // On Node 10, ASCII escape chars get 6 'f's like \\xe6 -> \\xffffffe6 and \\xf1 -> \\xfffffff1
    const replaceRegex = /\\(?!u[0-9A-Fa-f]{4})(?!xf{6}[0-9A-Fa-f]{2})/g
    // \u005c is the unicode escape for the \ char

    const escapedAscii = line.match(/\\xf{6}[0-9A-Fa-f]{2})+/g)
    if (escapedAscii) {
      for (const match of escapedAscii) {
        line = fixEscapedCharacter(line, match)
      }
    }
  ...
  if (nodeMajorV < 10) {
    ...
    const escapedChars = line.match(/(\\u[0-9A-Fa-f]{4}|\\x[0-9A-Fa-f]{2})+/g)
    if (escapedChars) {
      for (const match of escapedChars) {
        line = fixEscapedCharacter(line, match)
      }
    }
...
function fixEscapedCharacter (line, match) {
  // Normal buffer decoding, String.normalize() etc doesn't work here, brute force needed
  return line.replace(match, JSON.parse(`"${match}"`))
}
