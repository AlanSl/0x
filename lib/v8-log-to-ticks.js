...
// 2. Escape backslashes because V8's --preprocess -j treats :\\ etc as an escape pattern,
//    garbling Windows paths or regular expression definitions that contain similar patterns
// TODO use streams
async function prepareForPreprocess (isolateLogPath) {
  ...
  data = data.split('\n').filter((s) => s.length < 1024).map(escapeBackslashes).join('\n')
  await writeFile(dest, data)
  return dest
}

function escapeBackslashes (line) {
  ...
  const firstIndex = line.search(/\\(?!u[0-9A-Fa-f]{4})/)
  ...
  // Start at -6 as the previous character may itself be a unicode escape char
  const startPath = firstIndex < 6 ? 0 : firstIndex - 6
  ...
  const after = line.slice(0, startPath) + line.slice(startPath).replace(replaceRegex, '\\u005c')
